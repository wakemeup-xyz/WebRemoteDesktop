...
以下描述已提交的输入恢复整改代码；23场景离线套件通过，主线程和独立 scoped review 均确认 R4 归属缺口已关闭，原八项发现全部闭合。2026-09-08 已合入并 push main（`83a9f79`），本地 Signal/Host 重启后健康检查、Host 在线和新资源交付通过，tunnel 未变；完整证据见验收报告。已有 Viewer 页面需刷新加载新代码；真机、系统 IME、Quartz 和实际公网 Viewer 输入不由服务健康推断。
