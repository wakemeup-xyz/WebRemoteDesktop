...
- [x] **严格离线整改验收**：`scripts/mobile_input_interaction_acceptance.py --out PATH [--browser chromium|webkit]` 只加载本地源码、阻断所有请求，不读取凭据、不提供base-url、不启动服务。`scope=offline-synthetic`；场景失败exit1，缺浏览器运行依赖exit2并写`NOT RUN`；2026-09-08 Chromium 运行的20个场景均为 `PASS`，且独立核对所有 checks 与网络安全字段。结果见[Task4输入恢复与诊断验收](../superpowers/reports/2026-09-07-input-recovery-observability-acceptance.md)及此前的[整改验收报告](../superpowers/reports/2026-09-06-mobile-input-interaction-remediation-acceptance.md)。
...
以下描述整改分支已提交的代码，不表示main或运行服务已经更新；Task4 的离线 Chromium 交互/诊断验收已通过，主线程 whole-branch review、真机、公网及与新main的组合结果仍未验证。
...
- [x] Task4输入恢复与诊断离线验收：20个场景覆盖真实 click→blur→reset ACK→fresh input、重试/草稿入口、trace/timeout/deferred/blocked/release 负路径、窄屏与 root fullscreen、Terminal 和 safe artifact/network checks；全部 `PASS`。这仍是 offline-synthetic 证据，不等于真机、Quartz、系统 IME 或公网全链路验收。
