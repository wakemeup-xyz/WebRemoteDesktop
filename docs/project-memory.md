...
- When Cloudflare returns `Unauthorized: Tunnel not found`, diagnose and report the stale quick tunnel. The safe supervisor is observation-only and must not rotate or replace it automatically; an agent must not manually start/restart it or regenerate the URL without explicit user authorization.
