...
- 可运行只读检查：`bash scripts/terminal-runtime-check.sh`。它只读取本地 health/status、`/tmp/wrd-safe-current-url.txt` 和可选 metrics/env probe；不会调用 `stop-safe-wrd.sh`、重启脚本、`cloudflared` 或 `launchctl remove`
