...
**Interfaces:** 产出 `Input.getEffectiveInputGate() -> { allowed, blockedReasons, recovery }`，`Input.requestInputRecovery({ source: 'auto'|'user' }) -> boolean`；`KeyboardTransport.getPendingReset() -> { inputId, seq, leaseEpoch } | null`；`MobileTextInput.invalidateContext(reason)` 和 `confirmEmptyContextRecovery() -> boolean`。内部 raw ID 不进入 getDiagnosticState。恢复 cycle 和 gate 的字段/语义按 Spec §3，供 Task 2 调用。
