...
- [ ] **Step 4c（跨输入方式复审）:** document实体keydown不能仅预检后直发；实际接受的导航/可打印键/组合键通过既有context-change事务失效移动已接受历史（或严格同步cursor），覆盖收起→实体编辑→重开IME。忽略的本地输入事件、拒绝发送和单独modifier不能误清历史；keyup仍无条件进入原安全释放，保留Task5新增viewport门禁。用真实controller/transport及内存远端模型回归，无重复发送。
