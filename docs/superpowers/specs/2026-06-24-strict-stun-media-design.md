...
新的产品边界改为：Cloudflare Tunnel 只负责网页入口、认证、信令和诊断上报；远程桌面媒体必须尝试 WebRTC 直连，允许 `host`、`srflx`、`prflx` 候选，不允许 TURN `relay`，也不允许 Cloudflare/Socket.IO 媒体隧道。连接失败时必须给出明确失败状态和可诊断日志，不自动也不手动改走媒体 tunnel。
...
2. 媒体严格直连：视频和 WebRTC DataChannel 只允许 direct ICE 候选，禁止 `relay` 候选成为可用路径，禁止 `startTunnelRelay()` 作为自动或手动恢复路径。
...
1. 不使用 TURN、VPS、中继服务器、Cloudflare Calls 或任何 relay media 服务。
2. 不保留手动媒体 tunnel 作为用户可点的兜底按钮。
3. 不保证公司网、校园网、运营商 CGNAT、双重 NAT、UDP 严格封锁场景一定能连通。
4. 首期不修改 `aiortc` / `aioice` 内部 UDP 端口绑定逻辑；固定端口范围和端口映射作为增强方案单独评估。
5. 不把完整 SDP、JWT、密码、键盘输入内容或未脱敏候选端点写入诊断日志。
...
6. `networkMode=relay` 与 `networkMode=tunnel` 在 strict policy 下不可选择。UI 中隐藏或禁用，并显示“当前策略禁止媒体中继”。
7. `localStorage` 中已有的 `wrdNetworkMode=tunnel` 或 `relay` 进入页面时必须被修正为 `auto` 或 `stun`，同时记录 `policy-normalized-mode` 事件。
8. 如果任意本地或远端 candidate 类型为 `relay`，视为 `policy-violation-relay-candidate`，关闭 PeerConnection，上报诊断，页面明确报错。
9. 如果进入了 `startTunnelRelay()`、收到 tunnel frame、或发送 `relay-stream-control`，视为 `policy-violation-media-tunnel`，必须终止并上报。这是测试必须覆盖的硬边界。
...
3. `relay` / `tunnel` localStorage 在 strict policy 下被 normalize。
4. relay candidate 被识别为 `policy-violation-relay-candidate` 并自动上报。
5. tunnel media 入口被调用时触发 `policy-violation-media-tunnel`。
