...
1. `session`：页面会话、模式选择、attempt 标识。
2. `network`：浏览器网络环境、连接类型、IPv4/IPv6、STUN server 探测。
3. `ice`：candidate 收集、remote candidate 注入、pair 选择、重启记录。
4. `media`：首帧、0 FPS、连接成功后短时卡死。
5. `policy`：relay/tunnel 选择、模式归一化、禁止路径触发。
6. `error`：失败终止点、失败分类、用户可见提示。

建议补充的事件：

1. `network-profile`：记录 `navigator.connection`、`onLine`、平台、是否 VPN/代理可见信号。
2. `stun-probe-start`：记录使用了哪些 STUN URL、触发时机、attempt ID。
3. `stun-probe-result`：记录每个 STUN URL 是否拿到 `srflx`、耗时、超时/拒绝/解析失败原因。
4. `ice-candidate-gathered`：记录候选类型、协议、地址族、是否 mDNS、是否来自本地或远端。
5. `ice-candidate-error`：记录候选解析失败、addIceCandidate 失败、SDP 解析失败。
6. `ice-selected-pair`：记录最终 pair 类型、协议、地址族、选中耗时。
7. `media-first-frame`：记录首帧时间、首帧前等待时长、当前 resolution/fps。
8. `media-timeout`：记录 0 FPS 持续时长、最后一帧时间、当前 selected pair。
9. `socket-reconnect`：记录信令连接断开/重连次数和间隔。
10. `policy-warning`：记录 TURN 配置被忽略、手动 relay/tunnel 被选择等。
...
```json
{
  ...
  "latency": null,
  "traceSummary": {
    "lastEvent": "ice-connection-state",
    "lastError": "ice failed after restart",
    "retryCount": 2
  },
  "inputState": {
    "keyboardMode": "mac",
...
2. `persistDiagnostic()` 报告文件包含完整脱敏后的 `networkProfile`、`probeResults`、`candidateSummary`、`selectedCandidatePair`、`events` 和 `traceSummary`，不再只写 logs/latency/input。
...
6. 若诊断上传失败，后端返回可重试码与本地缓存建议，客户端将 report 暂存后在下一次 socket 连接成功时补发。
