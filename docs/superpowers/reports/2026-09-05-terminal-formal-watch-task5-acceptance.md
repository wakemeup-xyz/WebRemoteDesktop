...
| Safe tunnel executable-boundary behavior | PASS | `node --test scripts/tunnel-safety-fixture.test.js` — 5 passed, 0 failed; subprocesses execute `start-safe-wrd.sh`, `run-safe-quicktunnel.sh`, and `restart-safe-tunnel.sh` with fake PATH/temporary state, proving current `false/false` preservation, legacy `true/true` `print/disable/bootout`, inspection-failure preservation, diagnostic-only Unauthorized/unreachable/exit handling, and explicit restart `bootstrap/enable/kickstart` |
