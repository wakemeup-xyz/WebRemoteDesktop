...
- Ordinary startup now detects a loaded legacy quick-tunnel LaunchAgent and only issues `disable` plus `bootout` when its installed plist proves `RunAtLoad=true` and `KeepAlive=true`; a loaded current `false/false` job or unknown inspection state is preserved. The migration leaves `/tmp/wrd-safe-current-url.txt` untouched and never calls explicit start/restart/rotate actions.
- If legacy migration `disable` or `bootout` fails, startup reports the exact failed operation, stops before local service/tunnel startup, and does not emit `safe wrd ready`.
...
- When a live quick-tunnel PID has a newer URL in its log than the current/archive files, the supervisor may health-gate and atomically republish that candidate; it does not stop, kill, spawn, or rotate the live connector.
- Formal-watch lock publication records an initialization marker before PID metadata, reclaims abandoned empty/incomplete locks only after owner checks, preserves `EPERM`/unknown `kill -0` states, and fails closed on formal-owner `ps` inspection errors before managed submit.
...
| Formal watch, tunnel, and runtime scripts | PASS | `node --test scripts/*.test.js` — 111 passed, 0 failed |
| Safe tunnel executable-boundary behavior | PASS | `node --test scripts/tunnel-safety-fixture.test.js` — 7 passed, 0 failed; subprocesses execute `start-safe-wrd.sh`, `run-safe-quicktunnel.sh`, and `restart-safe-tunnel.sh` with fake PATH/temporary state, proving current `false/false` preservation, legacy `true/true` `print/disable/bootout`, migration failure stop-before-ready, live-PID URL republish without connector replacement, inspection-failure preservation, diagnostic-only Unauthorized/unreachable/exit handling, and explicit restart `bootstrap/enable/kickstart` |
| Formal-watch focused safety branches | PASS | `node --test scripts/watch-fixed-domain.test.js` — 22 passed, 0 failed; includes mkdir-before-PID crash reclaim, expired incomplete marker reclaim, EPERM lock preservation, token/formal-owner `ps` fail-closed, and no-submit assertions |
| Shell syntax | PASS | `bash -n scripts/watch-fixed-domain.sh scripts/lib-fixed-domain.sh scripts/run-safe-quicktunnel.sh scripts/lib-tunnel-launchctl.sh scripts/start-safe-wrd.sh scripts/restart-fixed-domain-tunnel.sh` — exit 0 |
| Patch whitespace | PASS | `git diff --check` — no output |

The script suite includes executable-boundary fixtures proving that inaccessible/Unauthorized/exited quick tunnels remain diagnostic-only, ordinary startup migrates only a positively identified legacy auto job without starting or deleting URL state, current/unknown plist states remain untouched, safe startup does not invoke tunnel start/rotate helpers, the LaunchAgent cannot auto-run, and the three active documents contain the same prohibition sentence. The Signal suite includes the 200ms default/config coverage and asynchronous PTY exit cleanup. Tests use fake PATH commands, temporary launchctl/plist state, or in-process harnesses; no live service, Host, Cloudflare connector, LaunchAgent, or tunnel was operated.
...
Automated tests do not substitute for the browser, physical-device, formal watcher, or public-path gates above. No service, Host, Signal Server, Cloudflare connector, quick tunnel, or LaunchAgent was started, stopped, restarted, rebuilt, or rotated for this acceptance; executable fixtures use fake commands and temporary files only.

**Commit message:** `fix(tunnel): close formal watcher safety gaps`
