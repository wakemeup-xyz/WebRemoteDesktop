...
- `signal-server` 通过构建产物 `web-client/dist/` 托管 Viewer：HTML 短缓存（60s must-revalidate，便于 CF 边缘 HIT），内容哈希 JS/CSS immutable；生产路径不直接托管 source `web-client/` 目录，也不依赖运行时 CDN
