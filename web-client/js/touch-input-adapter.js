(function attachTouchInputAdapter(root, factory) {
  ...
  function create(options) {
    ...
    const commitDown = (payload, reason) => {
    ...
    const flushMove = () => {
      ...
      if (move && enabled() && generation === frameGeneration) sendMouse('move', move);
    };
    ...
    const flushWheel = () => {
      ...
      pendingWheel = null;
      if (generation === frameGeneration) sendMouse('wheel', wheel);
    };
    ...
    function pointerdown(event) {
      ...
      pointers.set(event.pointerId, { startPoint: { ...point }, point: { ...point }, startedAt: clock(), startClientX: clientX, startClientY: clientY, clientX, clientY }); remember(point); element?.setPointerCapture?.(event.pointerId);
      if (pointers.size > 1) {
      ...
      pressTimer = setTimer(() => {
        ...
        const p = pointers.get(event.pointerId)?.startPoint;
        const id = p && commitDown({ ...p, button: 'right', clickCount: 1, buttons: 2 }, 'long-press-down-failed');
    ...
    function pointermove(event) {
      ...
      const id = commitDown({ ...p.startPoint, button: 'left', clickCount: 1, buttons: 1 }, 'drag-down-failed');
    ...
    function pointerup(event) {
      ...
      if (button) {
        ...
        if (generation !== entryGeneration) return;
        if (point) emit('up', { ...point, button, clickCount: 1, buttons: 0 }, 'pointer-up-failed');
        ...
        const id = commitDown(down, 'tap-down-failed');
        if (generation !== tapGeneration) return;
        if (id) emit('up', { ...point, button: 'left', clickCount, buttons: 0 }, 'tap-up-failed');
      }
